...
```mermaid
flowchart TD
    A[gRPC Client] -->|1. methodWithOptions| B[NestedService.methodWithOptions]
...
|------|------|------|
| 1 | gRPC Client | Call methodWithOptions RPC interface |
...
public Response methodWithOptions(Request request) {
    // Implementation code
}
```
...
try {
    ...
    // Call RPC method
    Response response = stub.methodWithOptions(request);
